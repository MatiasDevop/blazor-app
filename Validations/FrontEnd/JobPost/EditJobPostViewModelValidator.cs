...
public class EditJobPostViewModelValidator : AbstractValidator<EditJobPostAction>
{
    public EditJobPostViewModelValidator()
    {
        RuleFor(vm => vm.Title)
        ...
        RuleFor(vm => vm.RecruiterId)
            .Must(i => i != Guid.Empty)
            .WithMessage(
                "You Must Select a Recruiter. You May Manage Roles under User Manager in Your Company Profile.");
        ...
        RuleFor(vm => vm.ApplyUrl)
            .Must((action, s) => { return action.UseCpccApply || !string.IsNullOrWhiteSpace(action.ApplyUrl); })
            .WithMessage("Apply Url is Required")
            .Must((model, property) => { return model.UseCpccApply || BeAValidUrl(model.ApplyUrl); })
